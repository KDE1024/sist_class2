...
public class TableCreate extends JFrame implements ActionListener {
	...
	private void setTableName(String tName) {
		if(createQuery.toString().contains("create")) {
			JOptionPane.showMessageDialog(this, "create 쿼리가 이미 생성되어있습니다. 삭제 후 다시 만들어주세요");
			return;
		}//end if 
	...
	private void setColumn(String coulmnName, String dataType, String columnSize, boolean pk, boolean nn) {
		...
		column.append(coulmnName).append(" ").append(dataType);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource() == jbtnReset) {
			switch(JOptionPane.showConfirmDialog(this, "DDL 쿼리문을 삭제하시겠습니까?")) {
		...
		if(ae.getSource() == jbtnSetCol) {
			...
			if(dataType.equals("char") || dataType.equals("varchar2")) {
				if("".equals(jtfSize.getText().trim())) {
					JOptionPane.showMessageDialog(this, "컬럼크기 필수 입력.");
					jtfSize.requestFocus(); //커서 갖다두기
					return;
				}//end if 
			}//end if
			...
			if(jcbNN.isSelected() && jcbPmKey.isSelected()) {
					jcbPmKey.setSelected(false);//Primary key인 상태에서는 N.N을 해제한다.
			}//end if
